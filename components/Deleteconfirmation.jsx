...
const Deleteconfirmation = ({ confirm, setConfirm, handleDeleteSelected }) => {
  return (
    <>
      {confirm && (
        <div className='absolute w-[25%] h-[25%] left-[37.5%] top-[25%] bg-white border-2 flex flex-col justify-around '>
         ...
         items-center hover:bg-white hover:text-red-600 flex justify-center bg-red-600 text-white'
          ...
          <div className='relative w-full h-full flex justify-around items-center'>
            <Button  action='evet' setConfirm={setConfirm} handleDeleteSelected={handleDeleteSelected} bg={'bg-red-600 w-[25%] '}>
              Evet
            </Button>
            <Button action='hayir' setConfirm={setConfirm} bg={'bg-addcolor w-[25%] '}>
              Hayır
            </Button>
          </div>
        </div>
